...
platform_check_image_key() {
	local board=$(ipq806x_board_name)
        local upgradekey1=$( get_upgrade_key "$1" "7" )
        local upgradekey2=$( get_upgrade_key "$1" "8" )

        case "$board" in
                "w831u")
                        if [ "$upgradekey1" == "31080000" -a "$upgradekey2" == "01000000" ]; then
                                return 0
                        fi

                        ;;
                "we212x")
                        if [ "$upgradekey1" == "12020000" -a "$upgradekey2" == "01000000" ]; then
                                return 0
                        fi
...
platform_del_image_hdr(){
	local new_image="/tmp/firmware_new.img"
	dd if="$1" of="$new_image" skip=1 bs=256 2>/dev/null
}
...
platform_check_image() {
	...
	[ -f "$new_image" ] && rm -f "$new_image"

	( eval "platform_check_image_key \"$image_name\"" ) || {          
        	echo "upgrade image key error" >/dev/console
        	return 1                                               
	}

	platform_del_image_hdr "$image_name"

	image_name="$new_image"

	image_is_FIT "$image_name" || return 1

	image_has_mandatory_section "$image_name" ${mandatory_nand} && {\
		mandatory_section_found=1
	}

	image_has_mandatory_section "$image_name" ${mandatory_nor_emmc} && {\
		mandatory_section_found=1
	}

	image_has_mandatory_section "$image_name" ${mandatory_nor} && {\
		mandatory_section_found=1
	}
	...
	if [ $mandatory_section_found -eq 0 ]; then
        ...
        image_has_mandatory_section "$image_name" $ddr_section && {\
                image_contains "$image_name" ddr-$board || {\
                        image_contains "$image_name" ddr-$(to_upper $board) || {\
			return 1
			}
		}
	}
	for sec in ${optional}; do
                image_contains "$image_name" ${sec} || {\
			echo "Warning: optional section \"${sec}\" missing from \"$image_name\". Continue..."
		}
	done

	for sec in ${ignored}; do
		image_contains "$image_name" ${sec} && {\
			echo "Warning: section \"${sec}\" will be ignored from \"$image_name\". Continue..."
		}
	done

	image_demux "$image_name" || {\
		echo "Error: \"$image_name\" couldn't be extracted. Abort..."
		return 1
	}

	[ -f /tmp/hlos_version ] && rm -f /tmp/*_version
	dumpimage -c "$image_name"
	if [[ "$?" == 0 ]];then
		return $?
	else
		echo "Rebooting the system" >> /dev/console
		reboot
		return 1
	fi
}
...
platform_do_upgrade() {
	...
	for sec in $(print_sections ${image_name}); do
	...
	case "$board" in
	db149 | ap148 | ap145 | ap148_1xx | db149_1xx | db149_2xx | ap145_1xx | ap160 | ap160_2xx | ap161 | ak01_1xx | ap-dk01.1-c1 | ap-dk01.1-c2 | ap-dk04.1-c1 | ap-dk04.1-c2 | ap-dk04.1-c3 | ap-dk04.1-c4 | ap-dk04.1-c5 | ap-dk04.1-c6 | ap-dk05.1-c1 |  ap-dk06.1-c1 | ap-dk07.1-c1 | ap-dk07.1-c2 | ap-dk07.1-c3 | ap-dk07.1-c4 | ap-hk01-c1 | ap-hk01-c2 | ap-hk01-c3 | ap-hk01-c4 | ap-hk01-c5 | ap-hk01-c6 | ap-hk02 | ap-hk06 | ap-hk07 | ap-hk08 | ap-hk09 | ap-hk10-c1 | ap-hk10-c2 | ap-hk11-c1 | ap-hk12 | ap-hk14 | ap-ac01 | ap-ac02 | ap-ac03 | ap-ac04 | ap-oak02 | ap-oak03 | db-hk01 | db-hk02 | ap-cp01-c1 | ap-cp01-c2 | ap-cp01-c3 | ap-cp01-c4 | ap-cp02-c1 | ap-cp03-c1 | db-cp01 | db-cp02 | mp-emu | ap-mp02.1 | ap-mp03.1 | ap-mp03.1-c2 | ap-mp03.3 | ap-mp03.3-c2 | db-mp02.1 | db-mp03.1 | db-mp03.1-c2 | db-mp03.3 | db-mp03.3-c2 | w831u | we212x)
		for sec in $(print_sections ${image_name}); do
			flash_section ${sec}
		done
